...
configured; `--arch` is only needed on the first build. Dependency tracking is kbuild's; `kstep`
builds the kernel only if there is none or it predates its .config, and drops the kmod's objects
after a kernel reconfiguration, which kbuild misses. Edits to the config fragments or inside
...
def build_linux(b: Build, extra_config: Path | None = None, *, reconfigure: bool = True):
    ...
    # copy, not copy2: stamped with the build's time, which kernel_stale() compares with .config
    shutil.copy(b.linux / b.arch.image, b.dir / "kernel")
    shutil.copy(b.linux / "vmlinux", b.dir / "vmlinux")


# Never built, or reconfigured since it was built (an interrupted `make.py linux`, a .config edited
# or regenerated by hand): the image would boot a kernel that disagrees with the config the kmod
# is compiled against. CI restores a built tree cleaned of its objects; its image is newer than
