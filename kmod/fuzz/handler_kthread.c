...
enum kstep_kthread_state kstep_op_kthread_state(int id) {
  if (!kstep_op_is_valid_kthread_id(id) || !kstep_kthreads[id])
    return KSTEP_KTHREAD_DEAD;
  return kstep_kthread_get_state(kstep_kthreads[id]);
}

u8 kstep_op_kthread_create(int a, int b, int c) {
  char name[16];

  if (!kstep_op_is_valid_kthread_id(a) || kstep_kthreads[a])
    return 0;

  scnprintf(name, sizeof(name), "kt%d", a);
  kstep_kthreads[a] = kstep_kthread_create(name);
  return 1;
}
...
u8 kstep_op_kthread_bind(int a, int b, int c) {
  ...
  kstep_kthread_bind(kstep_kthreads[a], &mask);
  return 1;
}
...
u8 kstep_op_kthread_start(int a, int b, int c) {
  ...
  if (kstep_op_kthread_state(a) != KSTEP_KTHREAD_CREATED)
    return 0;
  kstep_kthread_start(kstep_kthreads[a]);
  return 1;
}
...
u8 kstep_op_kthread_yield(int a, int b, int c) {
  ...
  if (state != KSTEP_KTHREAD_SPIN && state != KSTEP_KTHREAD_YIELD)
    return 0;
  kstep_kthread_yield(kstep_kthreads[a]);
  return 1;
}
...
u8 kstep_op_kthread_block(int a, int b, int c) {
  ...
  if (state != KSTEP_KTHREAD_SPIN && state != KSTEP_KTHREAD_YIELD)
    return 0;
  kstep_kthread_block(kstep_kthreads[a]);
  return 1;
}
...
u8 kstep_op_kthread_syncwake(int a, int b, int c) {
  ...
  if (!kstep_kthreads[a] || !kstep_kthreads[b])
    return 0;
  ...
  kstep_kthread_syncwake(kstep_kthreads[a], kstep_kthreads[b]);
  return 1;
}
