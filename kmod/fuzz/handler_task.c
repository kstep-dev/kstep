...
/*
 * Validate that task @a exists and is currently on-cpu, as required by ops that
 * act on a running task. Returns the task, or NULL if the id/task is invalid
 * (caller should return 0). Panics with @verb if the task exists but is not
 * running, matching the original per-op panic messages.
 */
static struct task_struct *require_running_task(int a, const char *verb) {
  if (!kstep_op_is_valid_task_id(a) || !kstep_tasks[a].p)
    return NULL;
  if (!kstep_op_task_running(kstep_tasks[a].p))
    panic("Task %d is not on CPU when %s", a, verb);
  return kstep_tasks[a].p;
}
...
u8 kstep_op_task_pin(int a, int b, int c) {
  struct task_struct *p = require_running_task(a, "pinning");

  if (!p)
    return 0;
  kstep_task_pin(p, b, c);
  return 1;
}

u8 kstep_op_task_fifo(int a, int b, int c) {
  struct task_struct *p = require_running_task(a, "setting FIFO");
...
u8 kstep_op_task_cfs(int a, int b, int c) {
  struct task_struct *p = require_running_task(a, "setting CFS");
...
u8 kstep_op_task_pause(int a, int b, int c) {
  struct task_struct *p = require_running_task(a, "pausing");
...
u8 kstep_op_task_set_prio(int a, int b, int c) {
  struct task_struct *p = require_running_task(a, "setting priority");
